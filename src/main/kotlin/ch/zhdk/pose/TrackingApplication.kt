...
import ch.zhdk.pose.io.*
import ch.zhdk.pose.osc.OscPublisher
import ch.zhdk.pose.pipeline.*
...
object TrackingApplication {
    ...
    private fun createInputProvider(): InputProvider {
        return when (config.input.inputProvider.value) {
            ...
            InputProviderType.VideoInput -> VideoInputProvider(Paths.get("data/shibuya.mp4"))
    ...
    private fun createPipeline(): Pipeline {
        return when (config.pipeline.pipelineType.value) {
            ...
            PipelineType.LightOpenPose -> LightOpenPoseIEPipeline(config.pipeline, createInputProvider(), pipelineLock)
            PipelineType.LightOpenPoseTorch -> LightOpenPoseTorchPipeline(config.pipeline, createInputProvider(), pipelineLock)
        }
    }
}